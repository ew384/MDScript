...
DihedralTRAPPEA::DihedralTRAPPEA(LAMMPS *lmp) : Dihedral(lmp)
{
  writedata = 1;
}
...
DihedralTRAPPEA::~DihedralTRAPPEA()
{
  if (allocated) {
...
void DihedralTRAPPEA::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    p = k0[type]+k1[type]*(1.0+cos(phi)) + k2[type]*(1.0 - cos(2.0*phi)) + k3[type]*(1.0 + cos(3.0*phi));
    pd = k1[type] - 2.0*k2[type]*sin(2.0*phi)*siinv + 3.0*k3[type]*sin(3.0*phi)*siinv;
...
void DihedralTRAPPEA::coeff(int narg, char **arg)
{
...
void DihedralTRAPPEA::write_restart(FILE *fp)
{
...
void DihedralTRAPPEA::read_restart(FILE *fp)
{
  allocate();
...
void DihedralTRAPPEA::write_data(FILE *fp)
{
