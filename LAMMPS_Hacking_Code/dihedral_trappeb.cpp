...
DihedralTRAPPEB::DihedralTRAPPEB(LAMMPS *lmp) : Dihedral(lmp)
{
  writedata = 1;
}
...
DihedralTRAPPEB::~DihedralTRAPPEB()
{
  if (allocated) {
...
void DihedralTRAPPEB::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    p = k0[type]+k1[type]*cos(phi) + k2[type]*cos(2.0*phi) + k3[type]*cos(3.0*phi) + k4[type]*cos(4.0*phi);
    pd = k1[type] + 2.0*k2[type]*sin(2.0*phi)*siinv + 3.0*k3[type]*sin(3.0*phi)*siinv + 4.0*k4[type]*sin(4.0*phi)*siinv;
...
void DihedralTRAPPEB::coeff(int narg, char **arg)
{
...
void DihedralTRAPPEB::write_restart(FILE *fp)
{
...
void DihedralTRAPPEB::read_restart(FILE *fp)
{
  allocate();
...
void DihedralTRAPPEB::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ndihedraltypes; i++)
    fprintf(fp,"%d %g %g %g %g %g\n",i, k0[i], k1[i],k2[i],k3[i],k4[i]);
}
